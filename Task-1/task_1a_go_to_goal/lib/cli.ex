defmodule ToyRobot.CLI do
  ...
  def process_args(_) do
    IO.puts("Welcome to the [[ Task 1A ]] Toy Robot simulator!")
    print_help_message()
    receive_command()
  end
  ...
  defp execute_command(["stop" | params], robot) do
    ...
    # file object to write the each action taken by Toy Robot
    {:ok, out_file} = File.open("task_1a_output.txt", [:write])

    # spawn and register CLI server process
    # with 'listen_from_client' callback function
    pid = spawn_link(fn -> listen_from_client([], out_file) end)
  ...
  Writes the action taken with position of robot to a text (.txt) file.
  """
  def listen_from_client(state, out_file) do
    receive do
      {:toyrobot_status, robot_x, robot_y, robot_face} ->
        ...
        listen_from_client([{robot_x, robot_y, robot_face} | state], out_file)
    end
  end
