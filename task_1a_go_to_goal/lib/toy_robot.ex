defmodule ToyRobot do
  ...
  def path(%ToyRobot.Position{x: x, y: y, facing: facing} = robot, difference, cli_proc_name) do
    if difference != 0 do
      if difference > 0 do
        robot = move(robot)
        send_robot_status(robot, cli_proc_name)
        difference = difference - 1
        path(robot, difference, cli_proc_name)
      else
        robot = move(robot)
        send_robot_status(robot, cli_proc_name)
        difference = difference + 1
