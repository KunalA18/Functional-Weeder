...
defmodule ToyRobot do
  ...
  def rotate(%ToyRobot.Position{facing: facing} = robot, should_face, face_diff, cli_proc_name) do
    ...
    case should_face == facing do
      ...
      true ->
        #{:ok, robot} #tuple that is needed to be returned
        #	bot_status(robot, cli_proc_name)
        robot
    end
  end
  ...
  def navigate(%ToyRobot.Position{x: x, y: y, facing: facing} = robot, diff, cli_proc_name) do
    ...
    IO.puts(obs_ahead)
    #Avoidance Code!
    #while((x>=1 or x<=5) and (y>=1 or y<=5)) 
    if obs_ahead == true do
      if robot[:facing] == "north" do
      #case A 
       # x= x+1, y= y
        robot_a= Enum.map(robot, fn({x,y,facing}) -> {x+1,y,facing})     
        robot_a= Enum.into(robot_a, %{})
        diff_x = goal_x - x
        diff_y = @robot_map_y_atom_to_num[goal_y] - @robot_map_y_atom_to_num[y]
        sum_t1 = diff_x + diff_y
      #case B 
       # x=x-1, y= y
        robot_b= Enum.map(robot, fn({x,y,facing}) -> {x-1,y,facing})     
        robot_b= Enum.into(robot_b, %{})
        diff_x = goal_x - x
        diff_y = @robot_map_y_atom_to_num[goal_y] - @robot_map_y_atom_to_num[y]
        sum_t2 = diff_x + diff_y
      #case C 
       # x= x, y= y-1
        robot_c= Enum.map(robot, fn({x,y,facing}) -> {x,y-1,facing})     
        robot_c= Enum.into(robot_c, %{})        
        diff_x = goal_x - x
        diff_y = @robot_map_y_atom_to_num[goal_y] - @robot_map_y_atom_to_num[y]
        sum_t3 = diff_x + diff_y         
        
      if (sum_t1 < sum_t2 and sum_t1 < sum_t3) do
      	robot = robot_a
        should_face = "east"
      ...
      else if (sum_t2 < sum_t1 and sum_t2 < sum_t3) do
      	robot = robot_b      	
        should_face = "west"
        robot = rotate(robot,should_face , face_diff, cli_proc_name)
        {_, robot} = navigate(robot, diff_x, cli_proc_name)
      ...
      else (sum_t3 < sum_t2 and sum_t3 < sum_t1) do
      	robot = robot_a
        should_face = "south"
        robot = rotate(robot,should_face , face_diff, cli_proc_name)
        {_, robot} = navigate(robot, diff_x, cli_proc_name)
      ...
      #case A 
       # x= x, y= y-1
        robot_a= Enum.map(robot, fn({x,y,facing}) -> ({x,y-1,facing}))     
        robot_a= Enum.into(robot_a, %{})
        diff_x = goal_x - x
        diff_y = @robot_map_y_atom_to_num[goal_y] - @robot_map_y_atom_to_num[y]
        sum_t1 = diff_x + diff_y
